...
class AsyncAzureClient:
    ...
    async def create_project(self, name: str, description: str):
        ...
        response = await self.client.post("/_apis/projects?api-version=7.0",
                                          json=data)

        if response.status_code == 202:
            # since create response have just link of new project
            # we need to get project info to make our response
            get_project_response = await self.client.get(
    ...
    async def list_projects(self) -> Success | Error:
        ...
        get_response = await self.client.get("/_apis/projects?api-version=7.0")
    ...
    async def get_project(self, project_name: str):
        ...
        if response.status_code == 200:
            ...
            return Success(
                "Project found.", {"id": json_response["id"],
                                   "name": json_response["name"], "url": json_response["url"]},
                status_code=200)
        if response.status_code in [203, 401]:
            return Error(message="you have authorization problem, recheck your token.",
                         status_code=response.status_code)
    ...
    async def create_work_item(self, project_id: str, work_item_type: str, work_item_value: str):
        ...
        response = await self.client.post(f"/{project_id}/_"
    ...
    async def __get_work_item_id(self, project_name: str, work_item_title: str):
    ...
    async def update_work_item(self, project_name: str, work_item_title: str,
        ...
        work_item_id = await self.__get_work_item_id(project_name, work_item_title)
        ...
        response = await self.client.patch(f"/"
    ...
    async def get_work_item(self, project_name: str, work_item_title: str):
        ...
        response = await self.client.get(f"/{project_name}"
                                         f"/_apis/wit/workitems/{work_item_id}?api-version=7.0")
